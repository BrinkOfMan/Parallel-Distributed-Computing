...
int main(int argc, char ** argv)
{
  ...
  int count = 1;  // Global count of primes found so far
  int i;  // loop control
...
#pragma omp parallel for default(shared) private(i) 
  for (i = 2; i <= (prime_max-1)/2; ++i) 
    marked[i*2 + 1] = 0;

  int p = 2;  // holds an integer known to be prime
  
  while ((p*p-1)/2 <= (prime_max-1)/2)
    {
      int base = (p*p-1)/2;  // note: lower multiples of p are already marked

#pragma omp parallel for default(shared) private(i) 
      for (i = base; i <= (prime_max-1)/2; i += p)
	marked[i*2 + 1] = 1;

#pragma omp single
      while (marked[p+=(2*p+1)])  
	;
    } 
