    ...
    for(int i = 0; i < 10; i++){ // Repeat "forever"
      ...
      */
      ...
      while(retry){
        ...
        proc.send(GETCHOP, "L");
        rec = proc.recv(ACK);
        if (strlen(rec) == 0) {cout << "P" << my_rank << ": now possesses " << my_rank << endl; retry = 0;}
        else {cout << "P" << my_rank << ": retrying for " << my_rank << endl;}
        stosleep(rand()%75); // Baby delay
      }
      ...
      while(retry){
        ...
        proc.send(GETCHOP, "R");
        rec = proc.recv(ACK);
        if (strlen(rec) == 0) {cout << "P" << my_rank << ": now possesses " << ((my_rank+1) % nprocs-1) << endl; retry = 0;}
