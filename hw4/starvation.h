...
void printchop(string input, int *chop){
  int nprocs = 0;
  MPI_Comm_size(MPI_COMM_WORLD, &nprocs);
  cout << input << ":  ";
  for (int i = 0; i < nprocs-1; i++){ cout << "chop[" << i <<"] = " << chop[i] << " "; }
  cout << endl;
}
...
public:
  static int run() {
    ...
    int chop[nprocs - 1]; // Allocating #philosophers amount of chopsticks (also setting count to #procs)
    for (int i = 0; i < nprocs-1; i++){ chop[i] = NONE;}

    int waiting[nprocs - 1]; // Allocating #philosophers amount of waiting slots
    for (int i = 0; i < nprocs-1; i++){ waiting[i] = NONE; }
