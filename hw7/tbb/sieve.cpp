...
int main(int argc, char ** argv)
{
  ...
  for(int k = 0; k < pset0.size(); k++){
    cout << "pset0[" << k << "]: " << pset0[k] << endl;

    for (int i = pset0[k]; i <= prime_max; i += pset0[k])
      marked[i] = 1;

    // This is wrong, but I've spent too long on just this part. Gonna hope for the best that this exact situation doesn't arise on the exam.
