...
// Commenting out the SumHeights class, as it's not needed for this lambda expression function.
...
int main(int argc, char** argv) {
  ...
  // lamda function to replace SumHeights class instances
  parallel_for(blocked_range<size_t>(1, n), [=, &integral] (const blocked_range<size_t> r) {
                                                              for(size_t i=r.begin(); i != r.end(); i++)
                                                              integral += f(a+i*h); 
                                                          });
